...
from matplotlib.pyparsing_py2 import alphas
...
def smoSimple(dataMatIn,classLabels,C,toler,maxIter):
    ...
    iter = 0          #iter is to count the times alphas didn't change continuously
    while(iter < maxIter):
        alphaPairsChanged = 0
        for i in range(m):
            ...
            if ((labelMat[i]*Ei < -toler) and (alphas[i] < C)) or ((labelMat[i]*Ei > toler) and (alphas[i] > 0)):
                ...
                #set L and H to make sure that 'ai * yi + aj *yj = r' is in [0,C]X[0,C] area
                ...
                #update b according to KKT condition
                if (0 < alphas[i]) and (C > alphas[i]): b = b1
                elif (0 < alphas[j]) and (C > alphas[j]): b = b2
                else: b = (b1 + b2)/2.0
                alphaPairsChanged += 1
                print "iter: %d i:%d, pairs changed %d" % (iter,i,alphaPairsChanged)
        #if not updated, then begin to count
...
class optStruct:
    ...
    def __init__(self,dataMatIn,classLabels,C,toler,kTup):
        ...
        self.b = 0
        self.eCache = mat(zeros((self.m,2))) #the first column is a flag bit stating whether the eCache is valid
...
#heuristics for choosing j:select j to maximize step |Ei - Ej|
def selectJ(i,oS,Ei):
...
for i in range(len(labelArr)):
    classierror.append(abs(datMat[i]*mat(ws) + b - labelArr[i]))
print classierror
print max(classierror)
'''

#rbf: k(x,y) = exp((-||x-y||^2)/2*kTup^2)
